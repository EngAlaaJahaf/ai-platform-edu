  ...
  return (
    <div className={`grid gap-3 w-full lg:grid-cols-12 ${isChatFullscreen ? 'fixed inset-0 z-[70] p-3 grid-rows-1 max-w-none' : 'lg:h-[calc(100dvh-80px)] lg:grid-rows-1 w-full'}`}>
      ...
      <div className={`flex flex-col glass-panel overflow-hidden shadow-lg h-full min-h-[500px] ${isChatFullscreen ? 'lg:col-span-9 xl:col-span-9 col-span-1 rounded-none' : 'lg:col-span-9 xl:col-span-9 col-span-1 rounded-2xl'}`}>
        ...
        <div className="px-3 sm:px-4 py-2 border-b border-slate-200 dark:border-slate-800 theme-nav flex items-center justify-between gap-2 shrink-0 font-['Tajawal']">
          <div className="flex items-center gap-2.5 min-w-0 flex-1">
            ...
            <div className="min-w-0">
              <div className="flex items-center gap-2">
                <h3 className="font-extrabold text-xs md:text-sm theme-text-primary truncate">
                  {sessions.find(s => s.id === activeSessionId)?.title || 'المحادثة الأكاديمية التفاعلية RAG'}
                </h3>
                {activePrompt && (
                  <span className="hidden md:inline-flex text-[11px] font-bold text-emerald-600 dark:text-emerald-400 bg-emerald-500/10 px-2 py-0.2 rounded-full items-center gap-1 shrink-0">
          ...
          <div className="flex items-center gap-1 sm:gap-1.5 shrink-0">
            ...
            <div className="relative" ref={modelDropdownRef}>
              <button onClick={() => setIsModelDropdownOpen(!isModelDropdownOpen)} className="flex items-center gap-1.5 px-2 sm:px-2.5 py-1 rounded-lg theme-card-inner border text-xs font-bold theme-text-secondary hover:theme-text-primary transition cursor-pointer">
                <span className="w-2 h-2 rounded-full bg-emerald-500 animate-pulse shrink-0"></span>
                <span className="font-mono text-xs truncate max-w-[40px] sm:max-w-[110px] hidden sm:inline">{chatModel}</span>
                <ChevronDown className={`w-3 h-3 transition shrink-0 ${isModelDropdownOpen ? 'rotate-180' : ''}`} />
              </button>
              {isModelDropdownOpen && (
