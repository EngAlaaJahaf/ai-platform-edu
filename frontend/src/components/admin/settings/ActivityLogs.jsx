  ...
  return (
    <div className="max-w-5xl space-y-8 animate-fade-in pb-12">
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          <h2 className="text-2xl font-black text-white mb-2">سجلات النشاط (Audit Logs)</h2>
          <p className="text-sm text-slate-400">مراقبة تحركات النظام، عمليات تسجيل الدخول، وتغييرات الإعدادات</p>
        </div>
        <button
          onClick={handleClearLogs}
          className="px-4 py-2.5 rounded-xl border border-rose-500/30 text-rose-400 hover:bg-rose-500/10 text-sm font-bold transition flex items-center gap-2"
        >
              ...
              {logs.map((log, i) => (
                <tr key={log.id || i} className="hover:bg-white/[0.02] transition-colors">
                  ...
                  <td className="px-4 py-3">
                    <span className={`inline-flex px-2 py-0.5 rounded text-[10px] font-bold border ${getLevelBadge(log.level)}`}>
                      {log.action}
                    </span>
                  </td>
                  <td className="px-4 py-3">
                    <p className="text-sm text-slate-300 line-clamp-2" dir="auto">{log.details}</p>
                  </td>
