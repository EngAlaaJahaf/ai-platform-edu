...
export default function UserSettings({ 
  ...
  return (
    <div className="max-w-5xl space-y-8 animate-fade-in pb-12 font-['Tajawal']">
      ...
      <div className="theme-bg-card border border-slate-200 dark:border-slate-200 dark:border-slate-800 rounded-3xl overflow-hidden shadow-xl">
        <div className="overflow-x-auto">
          <table className="w-full text-right text-xs">
            <thead>
              <tr className="border-b border-slate-200 dark:border-slate-200 dark:border-slate-800 bg-slate-100/70 dark:theme-card-inner/70 theme-text-primary font-black">
                ...
                <tr key={user.id} className="hover:bg-slate-500/5 transition-colors">
                  <td className="px-5 py-4">
                    <div className="flex items-center gap-3">
                      <img src={user.picture || `https://api.dicebear.com/7.x/avataaars/svg?seed=${user.email}`} alt="" className="w-10 h-10 rounded-full border border-slate-300 dark:border-slate-700 bg-slate-100 dark:theme-card-inner" />
                      <div>
      ...
      {editingUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="theme-bg-card w-full max-w-2xl rounded-3xl border border-slate-200 dark:border-slate-200 dark:border-slate-800 shadow-2xl overflow-hidden flex flex-col max-h-[90vh]">
            ...
            <div className="p-6 overflow-y-auto flex-1">
              <form id="edit-user-form" onSubmit={handleUpdateUser} className="space-y-6">
                <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
                  <div>
                    <label className="block text-xs font-bold theme-text-muted mb-1">الاسم الكامل</label>
                    <input
                      type="text"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-bold theme-text-muted mb-1">البريد الإلكتروني</label>
                    <input
                      type="email"
                      required
                  ...
                  <div>
                    <label className="block text-xs font-bold theme-text-muted mb-1">الرتبة (Role)</label>
                    <select
                ...
                <div className="pt-4 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800">
                  <h4 className="text-sm font-bold theme-text-primary mb-4">صلاحيات الوحدات (Atom Level Control)</h4>
                  <div className="grid grid-cols-1 sm:grid-cols-2 md:grid-cols-3 gap-3">
                    {PERMISSION_OPTIONS.map(opt => (
                      <label key={opt.key} className="flex items-center gap-3 p-3 rounded-xl theme-card-inner border cursor-pointer hover:bg-slate-100/50 dark:theme-card-inner/50 transition">
                        <input 
                          type="checkbox" 
            ...
            <div className="p-4 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:theme-card-inner/50 flex justify-end gap-2">
          ...
          <div className="theme-bg-card border border-slate-200 dark:border-slate-200 dark:border-slate-800 w-full max-w-md rounded-3xl border shadow-2xl overflow-hidden flex flex-col">
            ...
            <div className="p-6">
              <form id="add-user-form" onSubmit={handleCreateUser} className="space-y-4">
                ...
                <div>
                  <label className="block text-xs font-bold theme-text-muted mb-1">كلمة المرور المؤقتة</label>
                  <input
                    type="text"
                    required
      ...
      {resetPassUser && (
        <div className="fixed inset-0 z-50 flex items-center justify-center bg-black/60 backdrop-blur-sm p-4">
          <div className="theme-bg-card border border-slate-200 dark:border-slate-200 dark:border-slate-800 w-full max-w-sm rounded-3xl border shadow-2xl overflow-hidden flex flex-col">
            <div className="px-6 py-4 border-b border-slate-200 dark:border-slate-200 dark:border-slate-800 flex items-center justify-between bg-slate-100/50 dark:theme-card-inner/50">
              <h3 className="text-lg font-black theme-text-primary flex items-center gap-2">
            ...
            <div className="p-6">
              <p className="text-sm theme-text-primary mb-4">كلمة المرور الجديدة لـ: <span className="font-bold theme-text-primary">{resetPassUser.name}</span></p>
              <form id="reset-pass-form" onSubmit={handleResetPassword}>
                <input
                  ...
                  className="w-full theme-card-inner border rounded-xl px-4 py-2.5 text-sm theme-text-primary focus:border-amber-500 transition font-mono"
                />
              </form>
            </div>
            
            <div className="p-4 border-t border-slate-200 dark:border-slate-200 dark:border-slate-800 bg-slate-100/50 dark:theme-card-inner/50 flex justify-end gap-2">
              <button onClick={() => setResetPassUser(null)} type="button" className="px-4 py-2.5 rounded-xl border border-slate-200 dark:border-slate-200 dark:border-slate-800 theme-text-primary hover:bg-slate-100/50 dark:theme-card-inner/50 text-sm font-bold transition">
                إلغاء
              </button>
