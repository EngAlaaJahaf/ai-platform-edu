...
> أُعِدَّت بتاريخ 2026-09-07 لتوجيه أعضاء الفريق الأربعة.
...
| 🎨 **إعادة تصميم الواجهة** (المسار 4) | نظام **«دراسة هادئة»**: فاتح/داكن متوازن، هالة الشيوس القصوى (AppShell + DOM خفيف)، تقسيم الشاشات الضخمة، نماذج معتمدة في `docs/ui-mockups/` |
...
## 3. تقسيم العمل — أربعة مسارات متكاملة
...
### المسار 4 — الواجهة وتجربة الاستخدام (`feat/ux-redesign`)
**مَن**: المسؤول عن إعادة تصميم الواجهة (المرجع: النماذج المعتمدة `docs/ui-mockups/index.html`).
**الهدف**: تحويل الواجهة المتكدّسة إلى نظام **«دراسة هادئة»** بثيمين متوازنين، وقسم شاشات ضخمة، وDOM خفيف.
**الملفات**: `frontend/src/index.css` (رموز + فاتح/داكن), `App.jsx`, جديد `AppShell.jsx`, `Header.jsx`, `ChatView.jsx`/`QuizView.jsx` (تقسيم), `SummaryView.jsx`, `TranslateView.jsx`, `ProofreadView.jsx`, `DashboardView.jsx`, `DocumentLibraryView.jsx`, `PresentationView.jsx`, المودالات، `admin/*`, `AuthGateView.jsx`, `SubscriptionView.jsx`, وحذف `App.css`.
**مهامه (ROADMAP)**: `P0` أساس التصميم، `P1` الهيكل (AppShell/DOM)، `P2` المحادثة، `P3` الاختبارات، `P4` الملخص+الترجمة+التدقيق، `P5` اللوحة+مستنداتي، `P6` العروض+المودالات، `P7` الأدمن+المصادقة+الاشتراك، `P8` التحقق الشامل.
**يفتح المهارة**: `eduai-phase4-ux-redesign`
...
2. **فروع مخصصة**: كل مسار ينشئ فرعه من `main` أحدث إصدار:
   ...
   git checkout -b feat/ux-redesign  # المسار 4
   ```
...
### 4.1 سحب ورفع التحديثات على GitHub (خطوة بخطوة)

**أ. عند بدء الجلسة — سحب آخر التحديثات من `main` (أهم عادة):**
```bash
git fetch origin          # تنزيل أحدث الحالات من GitHub دون تغيير ملفاتك
git checkout main         # الانتقال لفرع main
git pull origin main      # تطبيق تحديثات الزملاء على نسختك المحلية
# أنشئ فرعك منه إن بدأت جديداً، أو أدمجه في فرعك الحالي:
git checkout feat/security
git merge main            # دمج مستجدات main داخل فرعك، ثم اسمح بالتعارضات إن وقعت
```

**ب. بعد كل جلسة عمل — رفع تغييراتك المحلية:**
```bash
git status                # استعراض ما تَعدّل (أحمر = محلي لا عبوت ولا نُشر)
git add <الملفات المعنية>  # تدريج: git add backend/database.py أو git add . لتفعيل كل شيء
git commit -m "fix: ..."   # التزام محلي برسالة Conventional
git push origin feat/security   # رفع الفرع إلى GitHub
```

**ج. عند رفع الفرع لأول مرة** (لم يوجد بعد على GitHub):
```bash
git push -u origin feat/security   # -u يربط التتبع تلقائياً للمرات القادمة
# بعدها تصبح مجرد: git push
```

**د. أثناء العمل على محور — تحديث الفرع من أحدث `main` (لتجنّب تعارض الدمج لاحقاً):**
```bash
git fetch origin
git merge origin/main      # أو: git rebase origin/main للتاريخ أنظف (استخدمه بحذر)
```

**هـ. إن تعارض الملف عندما تدمج (CONFLICT):** افتح الملف، ستجد علامات `<<<<<<<`/`=======`/`>>>>>>>`؛ اترك النسخة الصحيحة واحذف العلامات، ثم:
```bash
git add <الملف>
git commit                # أو git rebase --continue بحسب تدفقك
```

**و. فتح PR:** بعد `git push` اتبع الرابط الذي يظهر في الطرفية، أو إن توفر `gh`:
```bash
gh pr create --base main --title "fix: ..." --body "أنجزت T2.3"
```

> **قاعدة ذهبية**: لا تعمل أبداً على `main` مباشرة — دائماً فرعك الخاص (`feat/...`) بل وتنقل التحديثات بالترتيب: `fetch` ← `merge main` ← `push`.
...
|---|---|---|---|---|
| 1 | pytest للـ API + ruff في CI | الجلسات الموقّعة T2.1 + إغلاق Google | جداول الفرق + كود الدعوة (T3.1) | أساس التصميم + الهيكل (P0-P1) |
| 2 | اختبارات الخدمات + vitest | فرض التوكنز + مفتاح الأدمن | مشاركة المستندات/العروض (أدوار) | المحادثة + الاختبارات (P2-P3) |
| 3 | تنظيف الموت + ErrorBoundary | مفاتيح API من الخادم + MIME | Deck Studio: صبغ art + خطوط + حد الترجمة | الملخص + الترجمة + التدقيق (P4) |
| 4 | ترحيلات DB + CI كاملة | rate-limit دائم + CORS | Docker + nginx + تحليلات حقيقية | اللوحة + مستنداتي + العروض (P5-P6) |
| 5+ | استكمال تغطية + مراجعة | توثيق أمان + مراجعة شاملة | توثيق النشر + اشتراكات | الأدمن + المصادقة + التحقق (P7-P8) |
...
- **المسار 4**: كل الشاشات منقّحة وفق «دراسة هادئة» (النموذج) بالثيمين؛ DOM ≤ 4 نوافذ؛ `ChatView`/`QuizView` مقسّمان؛ لا `text-[<12px]` في الأجسام ولا `rounded-3xl` للبطاقات؛ إحصاءات الأدمن الحية سليمة؛ لقطات قبل/بعد + `npm run build`/`lint` خضراء.
